...
async def basla():
    ...
    eklenen_say = 0
    for veri in gelen_veriler["WowzaYayin"]:
        if veri in mevcut_veriler.get("Marmaris", {}).get("WowzaYayin", []):
            continue

        mevcut_veriler.get("Marmaris", {}).setdefault("WowzaYayin", []).append(veri)
        eklenen_say += 1
