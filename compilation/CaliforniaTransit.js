	...
	return new Promise( ( resolve, reject ) => {
		...
		http.request( cam_string[0], ( res ) => {
			...
			res.on( 'end', () => {
				...
				let county = data.slice( data.search( /<title>.* : .* : .* : .*<\/title>/ ), data.indexOf( '</title>' ) ).split( ' : ' )[1];
				if ( !county ){
...
async function Compile ( data ){
	...
	for ( const cam_string of data ){
		try {
			const cam_info = await GetCamInfo( cam_string );
			const county = cam_info[0];
			const cam = cam_info[1];
