				...
				https.request(icam.url, (res) => {
					...
					res.on('end', () => {
						if (res.statusCode === 200){
							data = data.split('\n');
							var new_url = data.filter(line => (line.indexOf('SRC="/scanweb/Camera.asp') !== -1))[0];
							...
							https.request(request_hostname + new_url, (res) => {
								...
								res.on('end', () => {
									if (res.statusCode === 200){
										data = data.split('\n');
										var headers = data.filter(line => (line.indexOf('class="SectionHeader"') !== -1));
										var links =  data.filter(line => (line.indexOf('class="Phantom"') !== -1));
